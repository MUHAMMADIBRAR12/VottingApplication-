                ...
                <table id="example1" class="table table-bordered table-striped">                
                  ...
                  <tbody>
                    ...
                    $mainid=$_SESSION['user_id'];
                      $query="SELECT *,candidates.id as vid from candidates JOIN provincial_seats ON provincial_seats.pr_id=candidates.seat_id 
                      JOIN city ON  city.ci_id=provincial_seats.city_id  JOIN province ON  province.id=provincial_seats.province
                      JOIN parties ON  parties.prty_id=candidates.platform
                       where candidates.partie_id='$mainid' AND candidates.type=1";
                    $select=mysqli_query($conn,$query);
                while($row=mysqli_fetch_assoc($select)){
                      $image=(!empty($row['photo']))? '../parties/'.$row['photo'] : '../parties/profile.jpg';
                      ?>
                  <tr>
                    <td><?php echo $row['seat_id'].' '.$row['platform']?></td>
...
<?php include 'include/prov_candidate_modal.php'; ?>
